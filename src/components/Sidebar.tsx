...
export default function Sidebar() {
        ...
        <div className="flex flex-col items-center text-center">
          <img
            ...
            onError={(e) => {
              ...
              t.onerror = null;
              t.src = "https://placehold.co/160x160?text=Avatar";
            }}
          />
          <h2 className="text-xl font-semibold">{p.name}</h2>
          {p.title && (
            <p className="text-sm text-(--muted-text)">{p.title}</p>
          )}
        </div>
        {p.location && (
  ...
  {p.bio && (
  <p className="text-sm text-(--muted-text)">{p.bio}</p>
  )}
      </div>
  ...
  <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-(--muted-text)">Social</h3>
          <ul className="flex flex-wrap gap-2 text-(--muted-text)">
